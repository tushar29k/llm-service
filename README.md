...
- **Warmup** (`lazy_load: false` in `config.yaml`): warms the model at startup — loads the weights and runs one dummy token — so the first real request is fast instead of eating the full load cost. A failed warmup fails startup loudly, which is what you want: better to catch a bad model id at deploy time than on the first user request. Mock backend's warmup is a no-op.
